...
export function buildCirculation(api){
  ...
  for(let floor=0;floor<5;floor++){
    const y=floor===4?18.68:floor*4.65;
    for(const z of [42.55,56.65]){
      if(floor>0)for(const side of [-1,1]){platform(side*6.25,y,z,7.9,3.5);rail(side*2.32,z<50?43.55:z,y,z<50?1.45:3.5,true);}
      for(const side of [-1,1])landings.push({floor,x:side*5.18,z,w:4.55,d:3.15,y});
    }
    if(floor>0){
      for(const side of [-1,1]){rail(side*5.55,40.85,y,6.5);rail(side*5.55,58.4,y,6.5);}
  ...
  // East serves 1–2 and 3–4; west serves 2–3 and 4–RF. Each pair has
  // more than 3 m of unobstructed space beyond the balustrade ends.
  for(let floor=0;floor<4;floor++)for(const [bank,offset] of [6.35,4].entries()){
    const x=(floor%2?-1:1)*offset;
    const y0=floor*4.65,y1=floor===3?18.68:y0+4.65,z0=45.6,z1=53.65,steps=31,width=1.08;
    for(let i=0;i<steps;i++){
      const t=(i+.5)/steps,z=z0+(z1-z0)*t,y=y0+(y1-y0)*(i+1)/steps;
      box(steel,x,y-.075,z,width,.15,Math.abs(z1-z0)/steps+.008);
      box(M.gold,x,y+.004,z-Math.sign(z1-z0)*.115,width-.025,.012,.019,0,false);
      for(let k=-5;k<=5;k++)box(rubber,x+k*.09,y+.006,z,.012,.008,.2,0,false);
    }
    for(const [z,y] of [[z0-Math.sign(z1-z0)*.65,y0],[z1+Math.sign(z1-z0)*.65,y1]])box(steel,x,y-.045,z,1.08,.09,1.3);
    for(const dx of [-.73,.73]){
      ...
      for(const [z,y] of [[z0-Math.sign(z1-z0)*.65,y0],[z1+Math.sign(z1-z0)*.65,y1]]){box(M.concrete,x+dx,y+.2,z,.22,.45,1.3);box(rubber,x+dx,y+.9,z,.085,.07,1.3);}
    }
    const direction=bank===0?'up':'down';
    label(direction==='up'?'↑ UP':'↓ DOWN',x,y0+.3,z0-Math.sign(z1-z0)*1.35,.78,Math.PI,direction==='up'?'#37684b':'#466275','#fff6da');
    flights.push({lowerFloor:floor,upperFloor:floor+1,side:floor%2?'west':'east',x,width:1.2,z0,z1,y0,y1,direction,flatStart:44.3,flatEnd:54.95});
    for(const [z,y] of [[z0-Math.sign(z1-z0)*.65,y0],[z1+Math.sign(z1-z0)*.65,y1]])walkables.push({x,z,w:1.14,d:1.3,y});
  }
  // Lift doors face a dedicated 3 m waiting area, connected to the side gallery.
  const liftX=7.1,liftZ=59.05,floors=[0,4.65,9.3,13.95,18.68,-4.2,-8.4],landingDoors=[];
  const doorGlass=glazing.clone();doorGlass.color.set('#62afa7');doorGlass.transmission=.965;doorGlass.thickness=.06;doorGlass.attenuationColor.set('#66aaa4');doorGlass.attenuationDistance=2;
  for(const y of floors){
    if(y>0){platform(7.75,y,56.17,5.45,3.05);platform(9.68,y,59.36,1.65,3.35);rail(8.855,59.36,y,3.35,true);rail(5.4,57.7,y,.75);}
    landings.push({floor:floors.indexOf(y),x:7.1,z:56.15,w:3.2,d:3.0,y,lift:true});
    label(y<0?'B'+Math.round(-y/4.2):y===18.68?'RF':`${floors.indexOf(y)+1}F`,8.78,y+2.18,57.65,.54,Math.PI,'#67553e','#fff5d4');
    for(const dx of [-.92,.92])box(M.bronze,liftX+dx,y+1.24,liftZ-1.35,.11,2.48,.14);
    box(M.bronze,liftX,y+2.51,liftZ-1.35,1.95,.11,.14);
    const pair=[];
    for(const side of [-1,1]){const door=new THREE.Mesh(new THREE.PlaneGeometry(.88,2.42),doorGlass);door.position.set(liftX+side*(y===0?1.22:.44),y+1.3,liftZ-1.37);glassGroup.add(door);pair.push(door);}
    landingDoors.push(pair);
  }
  for(const dx of [-1.35,1.35])for(const dz of [-1.35,1.35])box(M.bronze,liftX+dx,6.75,liftZ+dz,.105,30.3,.105);
  for(const dx of [-1.28,1.28]){const p=new THREE.Mesh(new THREE.PlaneGeometry(2.64,30.1),glazing);p.rotation.y=Math.PI/2;p.position.set(liftX+dx,6.7,liftZ);glassGroup.add(p);}
  const rear=new THREE.Mesh(new THREE.PlaneGeometry(2.64,30.1),glazing);rear.position.set(liftX,6.7,liftZ+1.3);glassGroup.add(rear);
  for(const x of [liftX-.92,liftX+.92])box(steel,x,6.7,liftZ+1.04,.07,30.2,.1);
  box(M.wood,liftX,21.8,liftZ,2.9,.16,2.9);
  ...
  moving(M.darkMetal,1.105,1.34,-.1,.08,1.12,.48);for(let i=0;i<7;i++)moving(i===4?M.teal:M.bronze,1.054,.96+i*.16,-.1,.024,.09,.1);
  for(const x of [-1.12,1.12])moving(M.bronze,x,1.05,.1,.04,.04,1.5);
  ...
  const doors=[];
  for(const side of [-1,1]){const p=new THREE.Mesh(new THREE.PlaneGeometry(.87,2.42),doorGlass);p.position.set(side*.44,1.3,-1.23);cabinGlass.add(p);doors.push(p);}
  doors.forEach((door,i)=>door.position.x=(i?1:-1)*1.22);
  return {flights,walkables,landings,lift:{cabin,cabinGlass,doors,landingDoors,doorGlass,floors,x:liftX,z:liftZ,dockFloor:0,open:1}};
}
