...
export function buildParking(api){
  ...
  for(const [level,y] of [-4.2,-8.4].entries()){
    ...
    box(floor,39,y-.16,exitZ,10,.32,6.2);
    walkables.push({x:38.5,z:exitZ,w:10,d:5.8,y});
    for(const dz of [-3.1,3.1])box(concrete,39,y+1.85,exitZ+dz,10,3.7,.2);
  ...
  const vertices=[40.5,0,-20,47.5,0,-20,47.5,-8.4,44,40.5,0,-20,47.5,-8.4,44,40.5,-8.4,44];
  ...
  for(let i=0;i<32;i++){const z=-19+i*2,y=-(z+20)*8.4/64;box(M.ivory,44,y+.018,z,.06,.025,.75,0,false);}
